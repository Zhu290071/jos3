...
def index_order():
    """
    Define the order of indices in the thermal network matrix.

    Returns
    -------
    indexdict : dict of dict
        Nested dictionary where the first level keys are ``BODY_NAMES`` and
        second level keys are ``LAYER_NAMES`` mapping to their matrix index.
    order_count : int
        Total number of indexed nodes including the central blood node; use
        this to allocate matrices or arrays of the correct size.
    """
    ...
    for key in ["Head", "Pelvis"]:
        indexdict[key] = {
            ...
            "core": 1, "muscle": 1, "fat": 1, "skin": 1}

    for key in ["Neck", "Chest", "Back"]:
        indexdict[key] = {
            "artery": 1, "vein": 1, "sfvein": None,
            "core": 1, "muscle": None, "fat": None, "skin": 1}
...
def localarr(bf_cr, bf_ms, bf_fat, bf_sk, bf_ava_hand, bf_ava_foot):
    ...
    for i, bn in enumerate(BODY_NAMES):
        ...
        if not indexof["muscle"] is None:
            bf_local[indexof["muscle"], indexof["artery"]] = 1.067*bf_ms[i]  # art to ms
            bf_local[indexof["vein"], indexof["muscle"]] = 1.067*bf_ms[i]  # vein to ms
        if not indexof["fat"] is None:
            bf_local[indexof["fat"], indexof["artery"]] = 1.067*bf_fat[i]  # art to fat
            bf_local[indexof["vein"], indexof["fat"]] = 1.067*bf_fat[i]  # vein to fat

        # Only hand
        if i == 7 or i == 10:
            bf_local[indexof["sfvein"], indexof["artery"]] = 1.067*bf_ava_hand  # art to sfvein
        # Only foot
        if i == 13 or i == 16:
            bf_local[indexof["sfvein"], indexof["artery"]] = 1.067*bf_ava_foot  # art to sfvein
